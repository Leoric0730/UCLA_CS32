...
int main()
{
    ...
    dictfile.open("/Users/haoyuluo/Desktop/CS32/CS32_Project3/test.txt");
    if (!dictfile) {
...
//    int count = 0;
//    string word;
//    while (getline(dictfile, word) && count < MAXDICTWORDS){
//        dict[count] = word;
//        count++;
//    }
//    if (count > MAXDICTWORDS) return MAXDICTWORDS;
//    else return count;}
    return lexiconBuilderhelper(dictfile, dict, 0);
    }
//    string word;
//    if (!getline(dictfile,word)) return 0;
//    *dict = word;
//    if (!(dict[MAXDICTWORDS-1].size()==0)) return 1;
//
